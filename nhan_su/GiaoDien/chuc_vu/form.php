...
$action = $laSua ? '/nhan-su/chuc-vu/cap-nhat/' . (int)($duLieu['id'] ?? 0) : '/nhan-su/chuc-vu/luu';
?>
<form method="post" action="<?= bao_mat_chuoi($action) ?>">
  <input type="hidden" name="_csrf" value="<?= bao_mat_chuoi(csrf_tao()) ?>">
  <div class="card">
    <div class="card-header"><h3 class="card-title"><?= $laSua ? 'Cap nhat chuc vu' : 'Them chuc vu' ?></h3></div>
    <div class="card-body row g-3">
      <div class="col-md-4">
        <label class="form-label">Ma chuc vu</label>
        <input class="form-control<?= isset($loiTheoTruong['code']) ? ' is-invalid' : '' ?>" name="code" value="<?= bao_mat_chuoi((string)($duLieu['code'] ?? '')) ?>">
      </div>
      <div class="col-md-8">
        <label class="form-label">Ten chuc vu</label>
    ...
    <div class="card-footer d-flex gap-2">
      ...
      <a class="btn btn-outline-secondary" href="/nhan-su/chuc-vu">Huy</a>
    </div>
  </div>
</form>
